...
import com.dhaalves.divideae.config.KafkaSseConsumer;
import com.dhaalves.divideae.config.KafkaSseProducer;
...
class DivideAeKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/divide-ae-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/divide-ae-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/divide-ae-kafka/unregister"));
                return;
            }
        }
